...
export class BlockchainService implements OnModuleInit {
  ...
  private async initialize() {
    const mantleRpcUrl = this.configService.get<string>(
  ...
  async repayLoanOnChain(loanId: string, amount: bigint): Promise<string> {
    this.ensureConnected();
  ...
  async getLoanFromChain(loanId: string): Promise<OnChainLoanInfo> {
    ...
    return {
      ...
      amountRepaid: (
        BigInt(loan.amount) +
        BigInt(loan.outstandingAmount) -
        BigInt(loan.outstandingAmount)
      ).toString(), // Calculate from outstanding
  ...
  async isLoanOverdue(loanId: string): Promise<boolean> {
    ...
    return (
      currentTime > Number(loan.startTime) + Number(loan.duration) &&
      loan.status === 1
    ); // ACTIVE status
  }
  ...
  async lockCollateralOnChain(
    ...
    this.logger.log(
      `Collateral locked on-chain: loanId=${loanId}, collateralId=${collateralId}, tx: ${receipt.hash}`,
    );
  ...
  async unlockCollateralOnChain(collateralId: string): Promise<string> {
    ...
    const tx =
      await this.collateralVaultContract.unlockCollateral(collateralId);
    const receipt = await tx.wait();

    this.logger.log(
      `Collateral unlocked on-chain: collateralId=${collateralId}, tx: ${receipt.hash}`,
    );
  ...
  private async getTokenDecimals(tokenAddress: string): Promise<number> {
    ...
    try {
      ...
      this.logger.warn(
        `Failed to fetch decimals for token ${tokenAddress}, defaulting to 18: ${error.message}`,
      );
        ...
        try {
          ...
          if (parsed?.name === 'CollateralLocked') {
            ...
            if (
              ...
              if (formattedAmount >= expectedAmount * 0.99) {
                // 1% tolerance
                this.logger.log(
                  `Verified collateral deposit: ${formattedAmount} of ${token} (decimals: ${decimals})`,
                );
                return true;
              }
            }
