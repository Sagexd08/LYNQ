...
export class ReconciliationService {
  ...
  async reconcileLoans() {
    ...
    for (const loan of loans) {
      try {
        const onChainLoan = await this.blockchainService.getLoanFromChain(
          loan.onChainLoanId!,
        );
    ...
    this.logger.log(
      `Reconciliation complete: ${reconciled} updated, ${discrepancies} discrepancies`,
    );
  }
  ...
  private async reconcileLoan(
    ...
    const dbOutstandingWei = BigInt(
      Math.floor(Number(dbLoan.outstandingAmount) * 1e18),
    );
    ...
    const expectedOutstanding = parseFloat(
      formatUnits(expectedOutstandingWei.toString(), 18),
    );
