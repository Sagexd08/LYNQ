...
export class EventListenerService implements OnModuleInit, OnModuleDestroy {
  ...
  private async startListening() {
    try {
      ...
      const loanCoreAddress =
        this.configService.get<string>('LOAN_CORE_ADDRESS');
      ...
      if (!rpcUrl || !loanCoreAddress || !vaultAddress) {
        this.logger.warn(
          'Missing blockchain configuration, event listener not started',
        );
        return;
      }
  ...
  private async handleLoanCreated(data: any) {
    ...
    try {
      ...
      if (!existingLoan) {
        this.logger.log(
          `No matching DB loan for on-chain loan ${data.loanId}, may be a direct contract call`,
        );
      }
    } catch (error) {
      ...
      if (loan) {
        ...
        if (data.amount && loan.outstandingAmount) {
          ...
          const newOutstanding =
            currentOutstanding > repaidAmount
        ...
        if (Object.keys(updateData).length > 0) {
          ...
          this.logger.log(
            `Synced loan ${loan.id} repayment from on-chain event`,
          );
        }
      }
    } catch (error) {
  ...
  private async handleLoanLiquidated(data: any) {
    ...
    try {
      ...
      if (loan && loan.status !== 'LIQUIDATED') {
        ...
        this.logger.log(
          `Synced loan ${loan.id} status to LIQUIDATED from on-chain event`,
        );
      }
    } catch (error) {
