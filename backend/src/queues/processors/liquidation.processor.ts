...
export class LiquidationProcessor extends WorkerHost {
  ...
  async process(job: Job<LiquidationCheckJobData>) {
    ...
    if (daysOverdue <= 3) {
      this.logger.log(
        `Loan ${loanId} in grace period (${daysOverdue} days overdue)`,
      );
    ...
    // Execute liquidation
    this.logger.log(
      `Executing liquidation for loan ${loanId} (${daysOverdue} days overdue)`,
    );
    ...
    try {
      ...
      this.logger.error(
        `Liquidation failed for loan ${loanId}: ${error.message}`,
      );
      throw error;
    }
  }
