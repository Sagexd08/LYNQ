...
export class LiquidationSchedulerService implements OnModuleInit {
    ...
    async checkOverdueLoans() {
        ...
        while (true) {
            const overdueLoans: Array<{ id: string }> = await this.prisma.loan.findMany({
                where: whereClause,
                select: { id: true },
                orderBy: { id: 'asc' },
                take: batchSize,
            ...
            this.logger.log(`Processing batch of ${overdueLoans.length} overdue loans (total: ${totalProcessed + overdueLoans.length})`);
