...
export class MlAssessmentProcessor extends WorkerHost {
    ...
    async process(job: Job<MlAssessmentJobData>) {
        this.logger.log(`Processing ML assessment job ${job.id} for loan ${job.data.loanId}`);
        ...
        try {
            ...
            this.logger.log(`ML assessment completed for loan ${job.data.loanId}: score=${result.creditScore}`);
            ...
            this.logger.error(`ML assessment failed for loan ${job.data.loanId}: ${error.message}`);
            throw error;
        }
    }
}
