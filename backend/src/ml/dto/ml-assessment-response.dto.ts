...
export class MlAssessmentResponseDto {
    ...
    @ApiProperty({ description: 'Risk level', enum: ['VERY_LOW', 'LOW', 'MEDIUM', 'HIGH', 'VERY_HIGH'] })
    riskLevel: string;

    @ApiProperty({ description: 'Default probability (0-1)' })
    defaultProbability: number;

    @ApiProperty({ description: 'Recommended action', enum: ['APPROVE', 'APPROVE_WITH_CONDITIONS', 'MANUAL_REVIEW', 'REJECT'] })
    ...
    @ApiPropertyOptional({ description: 'Top factors influencing decision', type: [FactorExplanation] })
