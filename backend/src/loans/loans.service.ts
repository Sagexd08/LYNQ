...
export class LoansService {
  ...
  async createLoan(userId: string, dto: CreateLoanDto) {
    ...
    const collateralAmountWei = ethers.parseEther(
      (dto.collateralValueUsd || 0).toString(),
    );
  ...
  async activateLoan(
    ...
    const collateralAmount =
      typeof collateralData.amount === 'string'
    ...
    if (this.blockchainService.isBlockchainConnected()) {
      ...
      if (!loan.onChainLoanId) {
        throw new BadRequestException(
          'Loan does not have an on-chain ID. Cannot verify collateral deposit.',
        );
      }
      ...
      try {
        ...
        if (!isVerified) {
          throw new BadRequestException(
            'Collateral deposit not verified on-chain',
          );
        }

        this.logger.log(
          `Collateral verified on-chain for loan ${loanId}, tx: ${collateralData.txHash}`,
        );
      } catch (error) {
        const errorMessage =
        ...
        throw new BadRequestException(
          `Collateral verification failed: ${errorMessage}`,
        );
      }
    } else {
      // For MVP on testnet, require blockchain connection
      throw new BadRequestException(
        'Blockchain connection required for loan activation',
      );
    }
    ...
    try {
      ...
      if (loan.onChainLoanId) {
        activationTxHash = await this.blockchainService.activateLoanOnChain(
          loan.onChainLoanId,
        );
        this.logger.log(
          `Loan activated on-chain: ${loan.onChainLoanId}, tx: ${activationTxHash}`,
        );
      }
    } catch (error) {
      ...
      if (collateralLocked) {
        try {
          await this.collateralService.unlockCollateral(
            { loanId },
            loan.userId,
          );
          this.logger.warn(
            `Collateral unlocked due to activation failure for loan ${loanId}`,
          );
        } catch (unlockError) {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      this.logger.error(
        `Loan activation failed for ${loanId}: ${errorMessage}`,
      );
      throw error;
    }
    ...
    userMetadata.totalBorrowed =
      (userMetadata.totalBorrowed || 0) + Number(loan.amount);
  ...
  async repayLoan(loanId: string, dto: RepayLoanDto) {
    ...
    if (loan.onChainLoanId && dto.txHash) {
      if (!this.blockchainService.isBlockchainConnected()) {
        throw new BadRequestException(
          'Blockchain connection required for on-chain loan repayment',
        );
      }
      ...
      try {
        ...
        if (!onChainVerified) {
          throw new BadRequestException(
            'Repayment transaction not verified on-chain',
          );
        }
        ...
        throw new BadRequestException(
          `Repayment verification failed: ${errorMessage}`,
        );
      }
    } else if (loan.onChainLoanId && !dto.txHash) {
      throw new BadRequestException(
        'Transaction hash required for on-chain loan repayment',
      );
    }

    const totalOwed =
      Number(loan.amount) * (1 + Number(loan.interestRate) / 100);
    ...
    if (amountRepaid < 0 || amountRepaid > Number(loan.amount)) {
      ...
      throw new BadRequestException(
        'Invalid loan state: outstanding amount calculation error',
      );
    }
    ...
    if (
      ...
      try {
        await this.blockchainService.unlockCollateralOnChain(
          loan.onChainLoanId,
        );
        this.logger.log(`Collateral unlocked on-chain for loan ${loanId}`);
      } catch (error) {
        this.logger.error(
          `Failed to unlock collateral on-chain: ${error.message}`,
        );
    ...
    if (isFullyRepaid) {
      ...
      userMetadata.totalRepaid =
        (userMetadata.totalRepaid || 0) + paymentAmount;
    ...
    const totalRepaid =
      Number(loan.amount) - Number(loan.outstandingAmount) + paymentAmount;
    return {
      ...updatedLoan,
      paymentAmount,
      totalOwed,
