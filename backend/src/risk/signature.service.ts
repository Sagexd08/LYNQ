...
export class SignatureService {
    ...
    private initializeWallet() {
        const privateKey = this.configService.get<string>('RISK_SIGNER_PRIVATE_KEY')
    ...
    async generateApprovalSignature(data: RiskApprovalData): Promise<SignedApproval | null> {
        if (!this.wallet) {
        ...
        try {
            const signature = await this.wallet.signMessage(ethers.getBytes(approvalHash));
            ...
            this.logger.log(`Generated risk approval signature for loan ${data.loanId}`);
            ...
            this.logger.error(`Failed to generate approval signature: ${error.message}`);
            return null;
        }
    }

    private constructApprovalMessage(data: RiskApprovalData, expiresAt: Date): string {
    ...
    async verifyApprovalSignature(approvalHash: string, signature: string): Promise<boolean> {
        if (!this.wallet) {
            return false;
        }
