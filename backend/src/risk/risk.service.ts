...
export class RiskService {
  ...
  async evaluateLoanRisk(
    dto: RiskEvaluationDto,
    ...
    const finalRiskLevel = this.determineFinalRiskLevel(
      mlAssessment,
      fraudCheck,
    );
  ...
  private performFraudChecks(data: {
    ...
    if (data.totalTransactions > 0) {
      ...
      if (data.loanAmount > avgTxValue * 20) {
        reasons.push(
          'Loan amount significantly exceeds typical transaction size',
        );
        fraudScore += 0.25;
      }
    }
